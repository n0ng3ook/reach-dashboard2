...
function App() {
  ...
  useEffect(() => {
    fetch("https://nice-gold-lovebird-sock.cyclic.app/pets_price")
      .then((res) => res.json())
      .then((result) => {
        console.log(result);
        setPetPrice(result);
    ...
    fetch("https://nice-gold-lovebird-sock.cyclic.app/pets_price_chart")
      .then((res) => res.json())
      .then((result) => {
        console.log(result);
        setPetChart({
  ...
  return (
    <div>
      <h1>นภสินธุ์ เรืองฤทธิ์ 6403022</h1>
      <ul>
        {petPrices.map((pet) => (
          <li key={pet.id}>
